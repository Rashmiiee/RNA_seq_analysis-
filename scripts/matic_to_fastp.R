...
for (sample in sample_list) {
  if (sample %in% sample_list) {
    ...
    if (file.exists(read1) && file.exists(read2)) {
      ...
      output_forward_paired <- file.path(output_dir, paste0(outputPrefix, "_", sample, "_forward_paired.fq.gz"))
      output_reverse_paired <- file.path(output_dir, paste0(outputPrefix, "_", sample, "_reverse_paired.fq.gz"))
      
      # Run the rfastp function
      rfastp(
        read1 = read1,
        read2 = read2,
        outputFastq = paste0(outputPrefix, "_fastp"),
        trimTailRead1 = 3,
        trimTailRead2 = 3
      )
    } else {
